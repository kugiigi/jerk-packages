...
WebappWebview {
    ...
    property real findInPageMargin
...
//    }
    ...
    QtObject {
        ...
        function openLinkExternally(url) {
            ...
            if (webapp.settings.askWhenOpeningLinkOutside) {
                let _openDialog = askOpenExternallyDialogComponent.createObject(webview, { "url": url })
                _openDialog.confirm.connect(function() {
                        Qt.openUrlExternally(url)
                    }
                )
                if (webapp.wide) {
                    _openDialog.openNormal();
                } else {
                    _openDialog.openBottom();
                }
            } else {
                Qt.openUrlExternally(url)
            }
...
//    }
    ...
    function openUrlExternally(url, isTriggeredByUserNavigation) {
        if (webapp.settings.externalUrlHandling == 3) {
            ...
            _openExternalDialog.openExternally.connect(function() {
                    internal.openLinkExternally(url)
                }
            )
            if (webapp.wide) {
                _openExternalDialog.openNormal();
            } else {
                _openExternalDialog.openBottom();
            }
        } else {
            ...
            externalLinkWasBlocked()
        }
    }

    function externalLinkWasBlocked() {
        webapp.showTooltip(i18n.tr("External link has been blocked"), "BOTTOM", 3000)
    }
    ...
    function navigationRequestedDelegate(request) {
      ...
      if (request.action === WebEngineNavigationRequest.IgnoreRequest) {
            if ((isMainFrame && request.navigationType === WebEngineNavigationRequest.LinkClickedNavigation)
                    || webapp.settings.externalUrlHandling > 0)
            {
    ...
    Component {
        id: askOpenExternallyDialogComponent
        Sapot.AskOpenExternallyDialog {}
    }
    Component {
        id: askExternalLinkDialogComponent
        Sapot.AskExternalLinkDialog {}
    }
}
