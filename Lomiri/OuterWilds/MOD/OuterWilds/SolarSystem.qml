...
//~         readonly property int timerLimit: 1
        readonly property bool fastMode: true
//~         readonly property bool fastMode: false
        property int currentTimer: 0
        frequency: solarSystem.visible ? fastMode ? LiveTimer.Second
                                                : LiveTimer.Minute
                                : LiveTimer.Disabled
        function reset() {
            currentTimer = 0
            frequency = Qt.binding( function() { return solarSystem.visible ? loopTimer.fastMode ? LiveTimer.Second : LiveTimer.Minute : LiveTimer.Disabled } )
        }
        onTrigger: {
            if (currentTimer == timerLimit) {
                frequency = LiveTimer.Disabled
...
//~             readonly property real maxDivisor: units.gu(30)
//~             readonly property real minDivisor: units.gu(15)
...
//~                 displayOrbitLine: true
    ...
    OrbitingObject {
        ...
        endVisibility: 20
        bodyToOrbit: sunGlow
//~         horizontalOrbitOffset: -units.gu(15)
        horizontalOrbitOffset: -units.gu(27)
//~             displayOrbitLine: true
//~         orbitDurationOverride: divisorChange.duration / (fastMode ? 2 : 4) // Around 4 loops before supernova
...
//~         rotationDurationOverride: orbitDurationOverride // * 2
//~         rotationDirection: 1
//~         pathRotation: 45
