...
Rectangle {
    id: root
    // ENH164 - Launcher appearance settings
    readonly property color panelColor: shell.settings.useCustomLauncherColor ? shell.settings.customLauncherColor : "#111111"
    // ENH167 - Behavior changes for custom opacity/color of components
    //readonly property real colorOpacity: shell.settings.useCustomLauncherOpacity ? shell.settings.customLauncherOpacity : 0.95
    readonly property real progressThreshold: 0.3
    readonly property real defaultOpacity: 0.95
    /* Disabled in favor of hiding the Drawer behind the blur
    // Gradually set opacity of the Launcher based on Drawer's progress
    // Should be fully transparent when drawer is fully opened
    // Start with defaultOpacity so Drawer will always be a bit hidden behind the Launcher
    readonly property real colorOpacity: {
        let _opacity = shell.settings.useCustomLauncherOpacity ? shell.settings.customLauncherOpacity : defaultOpacity

        if (shell.settings.customLauncherOpacityBehavior) {
            if (drawerProgress === 0)
                return _opacity

            if (drawerProgress <= progressThreshold)
                return defaultOpacity

            return defaultOpacity * (1 - ((drawerProgress - progressThreshold) / (1 - progressThreshold)))
        }
    ...
    */
    readonly property real colorOpacity: {
        ...
        if (shell.settings.customLauncherOpacityBehavior) {
            ...
            return _opacity + ((drawerBGColor.a - _opacity) * (drawerProgress / 1))
        }

        return _opacity
    }
    property color drawerBGColor: "white"
    // ENH167 - End
    property bool lightMode : false
    // color: lightMode ? "#F2FEFEFE" : "#F2111111"
    // ENH167 - Behavior changes for custom opacity/color of components
    //color: shell.settings.useCustomLauncherColor || shell.settings.useCustomLauncherOpacity ? Qt.hsla(panelColor.hslHue, panelColor.hslSaturation, panelColor.hslLightness, colorOpacity)
    //            : lightMode ? "#F2FEFEFE" : "#F2111111"
    color: shell.settings.customLauncherOpacityBehavior && drawerProgress >= progressThreshold
                    ? Qt.hsla(drawerBGColor.hslHue, drawerBGColor.hslSaturation, drawerBGColor.hslLightness, colorOpacity)
    ...
    // ENH164 - End
    // ENH167 - Behavior changes for custom opacity/color of components
    property bool gestureDragging: false
    ...
    readonly property real panelBottomMargin: mainColumn.anchors.topMargin
    // ENH169 - End
    ...
    Column {
        ...
        anchors {
            fill: parent
            // ENH169 - Launcher bottom margin for rounded corners
            topMargin: shell.settings.enableLauncherBottomMargin && root.inverted && shell.isBuiltInScreen ? shell.settings.roundedCornerMargin : 0
            // ENH169 - End
        }
        ...
        Rectangle {
            ...
            color: {
                ...
                if (shell.settings.useCustomBFBColor) return shell.settings.customBFBColor
                // ENH032 - End
            ...
            Icon {
                ...
                //                                                                   : parent.width * .6
                width: {
                    if (shell.settings.useCustomeBFBLogoAppearance) {
               ...
               // ENH046 - End
                ...
                //                                                                                               : homeLogoResolver.resolvedImage
                source: {
                    if (shell.settings.ow_bfbLogo > 0) {
                    ...
                    return homeLogoResolver.resolvedImage
                }
                // ENH032 - End
                 // ENH046 - End
                // ENH023 - End
                ...
                color: shell.settings.useCustomeBFBLogoAppearance ? shell.settings.customLogoColor
                                                                  : "white"
                // ENH032 - End
                keyColor: "#ffffff"
                // ENH046 - End
