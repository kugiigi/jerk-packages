...
PullToRefreshStyle {
    ...
    Connections {
        ...
        function onContentYChanged() {
            ...
            // Fixes refresh when search field is displayed since we now use topMargin instead of anchors.topMargin
            // if (styledItem.target.originY - styledItem.target.contentY == 0 && willRefresh) {
            if (styledItem.target.originY - styledItem.target.contentY - styledItem.target.topMargin == 0 && willRefresh) {
            // ENH236 - End
                styledItem.refresh()
                willRefresh = false
            }
