...
Showable {
    ...
    Loader {
        ...
        Connections {
            ...
            // ENH032 - Infographics Outer Wilds
            function onOwToggle() { root.enableOW = !root.enableOW }
            function onFastModeToggle() { root.fastModeOW = !root.fastModeOW }
            // ENH032 - End
        }
    ...
    Rectangle {
		...
		Connections {
			target: biometryd
			function onFailed() { fpMarker.color = theme.palette.normal.negative }
			function onSucceeded() { fpMarker.color = theme.palette.normal.positive }
			function onStarted() { fpMarker.color = theme.palette.normal.activity }
		}
    }
    // ENH001 - End
