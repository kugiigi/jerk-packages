...
QtObject {
    ...
    // ENH243 - Virtual Touchpad Enhancements
    property bool touchpadMode: false
    // ENH243 - End
    ...
    function handlePressedChanged(pressed, pressedButtons, mouseX, mouseY) {
        if (pressed && pressedButtons === Qt.LeftButton) {
            ...
            if (root.touchpadMode) {
                pos = boundsItem.mapToItem(target, mouseX, mouseY);
            } else {
                pos = mapToItem(target, mouseX, mouseY);
            }
            // ENH243 - End
    ...
    function handlePositionChanged(mouse, sensingPoints) {
        if (priv.dragging) {
            ...
            let pos = Qt.point(0, 0);
            if (root.touchpadMode) {
                pos = boundsItem.mapToItem(target.parent, mouse.x, mouse.y);
            } else {
                pos = mapToItem(target.parent, mouse.x, mouse.y); // How can that work if we're just a QtObject (not an Item)?
            }
            // ENH243 - End
